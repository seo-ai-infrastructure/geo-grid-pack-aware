...
export async function finalizeVideo(sb, scanId, pointId, phoneId, label, recordOk = true) {
  ...
  try {
    ...
    const uploadStart = parseDuoCommandResult(await duoCommand(phoneId, uploadCmd));
    if (!uploadStart.ok && uploadStart.notMounted) {
      console.log(`[ScreenRecord] upload skipped, phone not mounted for ${label}`);
      return;
    }
