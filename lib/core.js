...
async function startLocalScreenRecord(phoneId, scanId, label, duration) {
  ...
  try {
    ...
    const cmd = `adb -s ${addr} shell "screenrecord --time-limit ${duration} --bit-rate ${env.RECORD_BITRATE} ${path} > /dev/null 2>&1 &"`;
    exec(cmd, () => {});
...
export async function startCloudScreenRecord(phoneId, scanId, label) {
  ...
  const cmd = `screenrecord --time-limit ${duration} --bit-rate ${env.RECORD_BITRATE} ${path} > /dev/null 2>&1 &`;
...
async function phoneFileReady(phoneId, path) {
  const res = await duoCommand(phoneId, `ls -l ${path}`);
  const content = res?.data?.content || '';
  const match = content.match(/\s(\d+)\s+\d{4}-\d{2}-\d{2}/);
  return match ? parseInt(match[1], 10) > 1000 : /geogrid_.*\.mp4/.test(content);
}
...
export async function kickPoint(sb, scan, point, phoneId, origin) {
  ...
  // Recording FIRST — wait for phone mount, then screenrecord before GPS settle and RPA
  ...
  // Upload runs in same waitUntil context; do not await (90s+)
